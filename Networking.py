        ...
        if nightly:
            ...
            if response.status_code == 200:
                ...
                if version != nightly_version:
                    log(f"A newer nightly version is available: {nightly_version}", False)
                    return True
                else:
                    log("You are using the latest nightly version.", False)
                    return False
            else:
                log("Failed to fetch version info.", False)
                False
        else:
            log("Checking release version... (not yet implemented)", False)
